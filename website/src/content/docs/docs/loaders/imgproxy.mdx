...
The `useImgproxyLoader` hook generates a URL for an image processed by imgproxy.

```tsx
import { useImgproxyLoader } from "@oh-image/react";

function MyImage() {
  const src = useImgproxyLoader("https://example.com/image.jpg", {
    resize: {
      width: 800,
    },
    format: "webp",
  ...
  return <img src={src} alt="Processed image" />;
}
```

## useImgproxyPlaceholder

The `useImgproxyPlaceholder` hook generates a low-quality placeholder URL for an image, useful for loading states or blur-up effects.

```tsx
import { useImgproxyPlaceholder } from "@oh-image/react";

function MyImage() {
  const placeholder = useImgproxyPlaceholder("https://example.com/image.jpg");
...
You can configure the loader globally using the `ImgproxyLoaderProvider`. This allows you to set the base URL for your imgproxy instance and default settings.
