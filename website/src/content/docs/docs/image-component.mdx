...
The `<Image>` component is a React component designed to render optimized images processed by the oh-image plugin. It handles responsive images, lazy loading, blur-up placeholders, and priority loading out of the box.
...
function App() {
  return <Image src={photo} alt="A photo" placeholder />;
}
```
...
| `placeholder` | `boolean` | No | Enable blur-up placeholder effect |
...
You can also pass a regular URL string, though you lose the automatic responsive images:
...
The `placeholder` prop enables a blur-up loading effect. A low-resolution blurred version of the image is shown immediately, then replaced with the full image once loaded.

```tsx
<Image src={photo} alt="Photo" placeholder />
```

### How it works

1. The plugin generates a small, blurred placeholder image during processing
2. The component sets this as a CSS background image
3. The full image loads on top
4. The transition creates a smooth blur-up effect

### Requirements

For the placeholder to work, the image must be processed with placeholder generation enabled. This can be done:
...
If `placeholder` is set to `true` but no placeholder URL is available, a console warning will be displayed.

---

## placeholderUrl

The `placeholderUrl` prop allows you to override the default placeholder image with a custom one.
...
<Image
  ...
  alt="Photo"
  placeholder
  placeholderUrl="/custom-placeholder.jpg"
/>
```
...
4. Calls `preload()` from React DOM to start fetching immediately
...
The `fill` prop makes the image fill its parent container, similar to `object-fit: cover` with `background-image`.
...
- `(max-width: 768px) 100vw` — On mobile, the image is 100% of viewport width
- `(max-width: 1200px) 50vw` — On tablet, the image is 50% of viewport width
- `33vw` — On desktop, the image is 33% of viewport width
...
The component uses the browser's default loading behavior. For most images, the browser will lazy load them automatically.
...
The `fetchPriority` prop hints to the browser how important this image is relative to other resources.
...
Here's an example combining multiple props:

```tsx
import { Image } from "@lonik/oh-image/react";
import heroImage from "./assets/hero.jpg?oh&placeholder=true";
import galleryImage from "./assets/gallery.jpg?oh&placeholder=true";
