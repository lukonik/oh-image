...
describe("ohImage plugin", () => {
  ...
  beforeEach(() => {
    ...
    (imageEntries.createImageEntries as Mock).mockReturnValue(
      mockImageEntriesInstance,
    );
  });
  ...
  describe("writeBundle hook", () => {
    it("should process and save all image entries", async () => {
      ...
      const mockEntriesInstance = (imageEntries.createImageEntries as Mock).mock
        .results[0].value;
      const entry = { origin: "test.jpg", width: 100 };
      mockEntriesInstance.entries.mockReturnValue(
        [["image-id.webp", entry]].values(),
      );

      await writeBundle();

      expect(imageProcess.processImage).toHaveBeenCalledWith("test.jpg", entry);
      expect(fileUtils.saveFileSafe).toHaveBeenCalledWith(
        join("/root/dist", "image-id.webp"),
        expect.any(Buffer),
      );
    });
  });
  ...
  describe("configureServer middleware", () => {
    ...
    it("should serve cached file if it exists", async () => {
      ...
      (fileUtils.readFileSafe as Mock).mockResolvedValue(
      ...
      const mockEntriesInstance = (imageEntries.createImageEntries as Mock).mock
        .results[0].value;
      mockEntriesInstance.get.mockReturnValue({
        origin: "origin.jpg",
        width: 100,
      });
    ...
    it("should 404/next if entry not found", async () => {
      ...
      const response = await mRequest().get(DEV_DIR + "unknown.webp");

      expect(response.status).toBe(404);
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Image entry not found"),
      );
